...
* Added "Frantic Mode" option for Announcer, which causes multikill lines to be played as they are triggered rather than waiting until the multikill extend timer ends
...
* Casing/civilian mode is now shown in assault phase label
* Added PONR indicator

* Motion Tracker is no longer "perfect" and now largely matches the style of the Halo: Reach era motion tracker, applying "ghost blip" afterimages on the Motion Tracker HUD display. You can revert this in settings, if you prefer my original prototype.
* Reticle Bloom is set to maximum for thhe duration of reloads, to match the style of Halo: Reach.
* Motion Tracker blips have new and improved assets, for far, close range, and elevated variants.
* Motion Tracker now takes into account whether an enemy is "intimidated" (or converted) and changes its blip color to match. (This may be inconsistent as client, due to PD2 netcode)
* Motion Tracker now shows drivable vehicles (Cars/Forklifts) and enemy vehicles (SWAT Turrets) on the map. This does not include stationary "prop" cars, static cop cars, or others, and may show invisible cars on some maps due to Overkill's level design. Color is determined by the team affiliation of occupant, if present.
* Motion Tracker now shows Sentries, both friendly and hostile. Hostile sentries are doubled in size, like vehicles.
* Your own Motion Tracker blip will not appear if you are standing still, and appears only intermittently when crouchwalking (cosmetic only, of course; does not affect detection)
* ECMs now apply a "jammer" effect to your own Motion Tracker, similar to Halo 3's Radar Jammer equipment or Halo Reach's Active Camo armor ability. You'll miss those eyes in the back of your head...

* Reorganized score/player information (in bottom right). Have a look! 
* Replaced Lives icon with generic Vitals icon, similar to the lives icon in Halo: Reach's Firefight.
* Added kill credit detection for vehicle collision kills, for the local player. 
* Added medals + announcer lines for Splatter Sprees
* Added medal for Splatter Kills
* Added medals + announcer lines for Wheelman Sprees (being the driver for a teammate who scores a kill)
* Added medal for Reload This!
...
* Replaced ammo bar texture to a regular rectangle instead of a super (badly) aliased and EXTRA THICC single bullet texture.
* Reticles can now be selected to appear by weapon type and firemode, in crosshair options. For example, you can make SMGs always use the Needler crosshair in singlefire mode and the Plasma Launcher in autofire mode, if you wanted to. 
* Added remaining missing crosshairs: (All created by myself from scratch, not taken from existing game assets)
* 	Added Halo Turret crosshair (default minigun category)
*	Added Halo 3 flamethrower crosshair (default flamethrower category)
* 	Added Halo Plasma Pistol crosshair (default bow category)
*	Added Halo Reach Needler crosshair
*	Added Halo Gravity Hammer crosshair
*	Added Halo Reach Target Designator crosshair
*	Added Halo Reach Focus Rifle crosshair
*	Added Halo Spiker crosshair
*	Added Halo Reach Concussion Rifle crosshair
*	Added Halo Fuel Rod Cannon crosshair
...
*	Added Halo Reach Plasma Launcher crosshair

*	Added Halo: Reach style waypoints for teammates, which have slightly different versions for Team AI and Player Teammates.
* Changed score banner texture to have a small indent in the bottom right corner, to better match Halo: Reach's style
* Score text is now aligned to the left instead of the right

* Added teammate health/armor display, as a percentage of 0-100.
* Added teammate condition icon, which changes depending on the player's state (eg. Swan Song icon, "Needs Revive" icon, Tased icon, Custody icon, etc.)
* Added mod option to hide the reticle while ADS.
* Added an Easter Egg, which can be activated during heists, somehow... ;)
* Added Drop-In Prompt panel

	Fixes
* Fixed shield sounds not always playing
* Fixed crashing when sending/receiving messages in preplanning screen
